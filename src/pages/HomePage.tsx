...
export default function HomePage() {
  ...
  const visibleLanguages = portal.languages?.length
    ? LANGUAGES.filter((l) => portal.languages.includes(l.code))
    : LANGUAGES;
  ...
  const filtered = allWords
    .filter((w) => {
      return (!langFilter || w.language === langFilter)
    ...
    <div className="max-w-6xl mx-auto px-5 sm:px-8">
      ...
      <section className="py-16 sm:py-24 text-center max-w-3xl mx-auto">
        ...
        <h1 className="font-serif text-4xl sm:text-5xl md:text-6xl font-semibold text-ink leading-[1.1] mb-6">
          {portal.theme?.tagline || t("app.tagline")}
        </h1>
      ...
      {/* Language cards — hide for single-language portals */}
      ...
      {/* Sticky toolbar: search + add + alphabet */}
      <StickyToolbar letters={letters} words={allWords} onSelectWord={(w) => setSelectedWord(w)} onAddNew={() => setShowNewWord(true)} />
      ...
      {loading ? (
        ...
        <div id="words-section" className="space-y-12 pb-16">
          {letters.map((letter) => (
            <section key={letter} id={`letter-${letter}`}>
              ...
              <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                {grouped[letter].map((word) => (
                  <WordCard key={word.id} word={word} showStatus={word.status !== "approved"} onDelete={isModerator ? (id) => setDeleteId(id) : undefined} onClick={(w) => setSelectedWord(w)} />
                ))}
              </div>
            </section>
          ))}
